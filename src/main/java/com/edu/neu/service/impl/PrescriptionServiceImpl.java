...
public class PrescriptionServiceImpl implements PrescriptionService {
    ...
    @Override
    public DataVO<PrescriptionVO> findAllByRegistId(Integer id, Integer page, Integer limit) {
        DataVO<PrescriptionVO> dataVO = new DataVO<>();
        dataVO.setCode(0);
        dataVO.setMsg("");
        IPage<Prescription> itemsIPage = new Page<>(page, limit);
        IPage<Prescription> result = prescriptionMapper.selectPage(itemsIPage, new QueryWrapper<Prescription>().eq("regist_id",id));
